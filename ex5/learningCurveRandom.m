...
% ---------------------- Sample Solution ----------------------
    ...
    for i = 1:m
        for j = 1:50        
            ...
            train_randoms = randperm(m);
            val_randoms = randperm(r);
            disp(train_randoms);
            disp(val_randoms);
            
            Xtrain_rand = X(train_randoms(1:i),:);
            ytrain_rand = y(train_randoms(1:i));
            % disp(Xtrain_rand);
            % disp(ytrain_rand);
            
            [theta] = trainLinearReg(Xtrain_rand, ytrain_rand, lambda);
            [Jtrain] = linearRegCostFunction(Xtrain_rand, ytrain_rand, theta, 0);
            error_train(i) = error_train(i) + Jtrain;
            
            Xval_rand = Xval(val_randoms(1:i),:);
            yval_rand = yval(val_randoms(1:i));
            % disp(Xval_rand);
            % disp(yval_rand);

            [Jval] = linearRegCostFunction(Xval_rand, yval_rand, theta, 0);
